(function () {
  ...
  function initYoonowSoftScroll() {
    var isFinePointer = window.matchMedia('(hover: hover) and (pointer: fine)').matches;
    if (reducedMotion || !isFinePointer) return;

    var scrollingElement = document.scrollingElement || document.documentElement;
    var currentY = window.scrollY || scrollingElement.scrollTop || 0;
    var targetY = currentY;
    var rafId = null;
    var lastWheelTime = 0;
    var maxScroll = 0;
    var nativeTargets = 'input, textarea, select, button, video, [data-native-scroll], .mobile-nav, .project-filter-tabs, .project-filter-scroll, .file-upload-control';
    ...
    function shouldUseNativeScroll(target) {
      ...
      if (target.closest && target.closest(nativeTargets)) return true;
      ...
      while (node && node !== document.body && node !== document.documentElement) {
        ...
        var canScrollY = (overflowY === 'auto' || overflowY === 'scroll') && node.scrollHeight > node.clientHeight + 2;
        var canScrollX = (overflowX === 'auto' || overflowX === 'scroll') && node.scrollWidth > node.clientWidth + 2;
    ...
    function stopSoftScroll() {
      ...
      targetY = window.scrollY || scrollingElement.scrollTop || 0;
      document.documentElement.classList.remove('soft-scroll-active');
    }

    function animateSoftScroll() {
      updateMaxScroll();
      currentY = window.scrollY || scrollingElement.scrollTop || 0;
      ...
      window.scrollTo(0, currentY + distance * 0.14);
      rafId = window.requestAnimationFrame(animateSoftScroll);
    }

    window.addEventListener('wheel', function (event) {
      if (event.ctrlKey || event.metaKey || event.shiftKey || shouldUseNativeScroll(event.target)) return;
      ...
      delta = Math.max(-520, Math.min(520, delta));

      targetY = Math.max(0, Math.min(maxScroll, targetY + delta * 0.92));
      document.documentElement.classList.add('soft-scroll-active');
      if (!rafId) rafId = window.requestAnimationFrame(animateSoftScroll);
    }, { passive: false });

    window.addEventListener('keydown', stopSoftScroll, { passive: true });
    window.addEventListener('resize', updateMaxScroll, { passive: true });
    window.addEventListener('blur', stopSoftScroll, { passive: true });
    window.addEventListener('scroll', function () {
      if (!rafId) targetY = window.scrollY || scrollingElement.scrollTop || 0;
    }, { passive: true });

    updateMaxScroll();
  }

  initYoonowSoftScroll();
